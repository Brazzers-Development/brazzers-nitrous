...
$(document).on('click', '#save', function(){
    $.post('https://brazzers-nitrous/save', JSON.stringify({
...
$(document).on('click', '#reset', function(){
    $.post('https://brazzers-nitrous/reset');
});
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('https://brazzers-nitrous/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('https://brazzers-nitrous/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('https://brazzers-nitrous/SetStancer', JSON.stringify({
...
QBTuner.Open = function() {
    $.post('https://brazzers-nitrous/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
QBTuner.Close = function() {
    ...
    $.post('https://brazzers-nitrous/exit');
}